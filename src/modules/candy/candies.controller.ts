...
import { candyService } from './candies.service'

export class CandyController {

  async listDoces(req: Request, res: Response) {
  ...
  async getDoce(req: Request, res: Response) {
    const { id } = req.params
  ...
  async createDoce(req: Request, res: Response) {
    const { name, description, price } = req.body
    const retornoDoce = await candyService.create(name, description, price)
    if (retornoDoce)
      res.status(500).json({ status: 500, message: 'something went wrong' })
    res.status(201).json(retornoDoce)
  }

  async updateDoce(req: Request, res: Response) {
  ...
  async deleteDoce(req: Request, res: Response) {
    const { id } = req.params
    const doce = await candyService.delete(id)
    if (Object.keys(doce).length === 0 || Object.keys(doce).includes('code')) {
      res.status(400).json({ status: 400, message: ' wrong code' })
    } else {
      res.status(200).json(doce)
